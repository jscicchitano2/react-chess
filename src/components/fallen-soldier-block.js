  ...
  render() {
    return (
      <div>
        ...
        )}{this.renderSquare("", this.props.whiteFallenSoldiers.length, this.props.player1Score, "points")}</div>
        <div className="board-row">{this.props.blackFallenSoldiers.map((bs, index) =>
          this.renderSquare(bs, index)
        )}{this.renderSquare("", this.props.blackFallenSoldiers.length, this.props.player2Score, "points")}</div>
      </div>
    );
  }
}