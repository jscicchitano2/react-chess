...
export default class MultiGame extends React.Component {
  constructor(props) {
      super(props);
      var whitePlayer = this.props.whitePlayer;
      var player = whitePlayer === this.props.player ? 1 : 2;
      this.state = {
        whitePlayer: player,
        moved: false,
        squares: initializeBoard(player),
        player: player,
        sourceSelection: -1,
        status: '',
        turn: 'white',
        ...
        blackFallenSoldiers: [],
        lastMove: [null, null],
        history: [initializeBoard(player)],
        stepNumber: 0,
        timerPlayer1: null,
        timerPlayer2: null,
        player1Score: 0,
  ...
  handleClick(i) {
    if (this.state.gameOver || this.state.stepNumber !== this.state.history.length - 1 || this.state.player !== 1) return;
  ...
  serialize(squares) {
    ...
    for (let sq = 0; sq < 64; sq++) {
        pieces.push(squares[sq] ? squares[sq].constructor.name : null);
    }
    return pieces;
  }

  deserialize(squares, pieces) {
    var newSquares = [];
    for (let sq = 0; sq < 64; sq++) {
        if (squares[sq]) {
            var p = squares[sq].player === 1 ? 2 : 1;
            var w = this.state.whitePlayer;
            var piece = pieces[sq];
            if (piece === "Pawn") {
                newSquares.push(new Pawn(p, w));
            } else if (piece === "Rook") {
                newSquares.push(new Rook(p, w));
            } else if (piece === "Knight") {
                newSquares.push(new Knight(p, w));
            } else if (piece === "Bishop") {
                newSquares.push(new Bishop(p, w));
            } else if (piece === "Queen") {
                newSquares.push(new Queen(p, w));
            } else {
                newSquares.push(new King(p, w));
            }
        } else {
  ...
  shareMove() {
    ...
    var pieces = this.serialize(squares);
  ...
  newGame() {
    if (this.state.timerPlayer1 === null) return;
